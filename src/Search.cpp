...
// RDB 
//  1/99 - Created
//  5/07 - Added binary checkpoint files (thanks to Chad Seys)
//  1/08 - Added multithreaded evaluation
//
// TO DO
...
ofstream statsout("evo_stats.out");
...
void TSearch::SetRandomSeed(long seed) {
    rs.SetRandomSeed(seed);
    statsout << "Random Seed: " << rs.GetRandomSeed() << endl;
}
...
void TSearch::SetVectorSize(int NewSize)
{
    ...
    FoldVector.SetSize(NewSize);
    FoldVector.FillContents(1);
}
...
void TSearch::SetPopulationSize(int NewSize)
{
    ...
    PopulationNodes.SetSize(NewSize);
    #endif
    for (int i = 1; i <= NewSize; i++)
...
// Set the search fold vector (whether parameters should be "folded" back into
// the search parameter values

void TSearch::SetSearchFold(TVector<int> &fold) 
{
	if (fold.Size() != vectorSize) {
		cerr << "Invalid vector size for SearchFold: " << fold;
		exit(0);
	}
	FoldVector = fold;
}

void TSearch::SetSearchFold(int flag) 
{
	FoldVector.FillContents(flag);
}
...
void TSearch::DoSearch(int ResumeFlag)
{
	...
	if (!SearchInitialized) InitializeSearch();
        ...
        treeNodeVal temp;
        for (int i = 1; i <= Population.Size(); i++) {
            temp.individual = Population[i]; temp.generation = Gen;
            PopulationNodes[i] = FamilyTree.insert(FamilyTree.begin(), temp);  
        }
        #endif
...
void TSearch::UpdatePopulationStatistics(void)
{
	...
	if (MaxPerf > BestPerf)
	{
        ...
        #ifdef TRACK_LINEAGES
        BestLeaf = PopulationNodes[bestindex];
        #endif
}
...
void TSearch::DisplayPopulationStatistics(void)
{
	if (PopulationStatisticsDisplayFunction != NULL)
            ...
            statsout << "Generation " << Gen << ": Best = " << BestPerf;
            statsout << ", Average = " << AvgPerf << ", Variance = " << PerfVar << endl;
	}
}
...
void TSearch::EvaluatePopulation(void)
{
#ifdef THREADED_SEARCH  // Evaluate the population in parallel
  // Create threads
  if (THREAD_COUNT > 1) {
    int NumIndividuals = PopulationSize()/THREAD_COUNT;
    pthread_t threads[THREAD_COUNT-1];
    PopRangeSpec psrs[THREAD_COUNT-1];
    int rc;
    for (int i = 1; i <= THREAD_COUNT - 1; i++) {
      psrs[i-1].search = this;
      psrs[i-1].start = (i-1)*NumIndividuals + 1;
      psrs[i-1].end = i*NumIndividuals;
    ...
    for (int i = (THREAD_COUNT - 1)*NumIndividuals + 1; i <= PopulationSize(); i++)
...
void TSearch::MutateVector(TVector<double> &v)
{
	...
	for (int i = 1; i <= vectorSize; i++) {
	    if (FoldVector[i])
	        v[i] = fold(v[i] + magnitude * TempVector[i],MinSearchValue,MaxSearchValue);
	    else if (ConstraintVector[i])
...
void TSearch::ReproducePopulationHillClimbing(void)
{
	...
	TVector<TVector<double> > Parent(1, psize);
	for (int i = 1; i <= psize; i++)
	    Parent[i].SetSize(vectorSize);
        #ifdef TRACK_LINEAGES
        TVector<treeNode> ParentNodes(1, psize);
        #endif
	...
	for (int i = 1; (i <= psize) && (j <= psize); i++) {
		sum += psize * fitness[i];
		while (rand < sum) {
                    Parent[j] = Population[i];
                    #ifdef TRACK_LINEAGES
                    ParentNodes[j] = PopulationNodes[i];
                    #endif
                    ParentPerf[j] = Perf[i];
                    j++;
                    rand++;
	...
	// Replace the current population with the parents
	Population = Parent;
        // If we're re-evaluating parents
	if (ReEvalFlag) {
            // reset BestPerf
            BestPerf = -1;
            // re-evaluate the parents and update their performance measures
            EvaluatePopulation();
            ParentPerf = Perf;
        }
	// Fill the population with children by mutating the parents
	for (int i = 1; i <= psize; i++)
	...
	for (int i = 1; i <= psize; i++)
	{
            // Keep the better individual in each parent/child pair
            // If the parent is better than the child
            ...
            #ifdef TRACK_LINEAGES
            else { // If a new child was created
                // Update the family tree
                treeNodeVal temp;
                temp.individual = Population[i]; temp.generation = Gen;
                PopulationNodes[i] = FamilyTree.append_child(ParentNodes[i], temp);
            }
            #endif 
	}
}
...
void TSearch::ReproducePopulationGeneticAlgorithm(void)
{
	...
	// Calculate population fitness
	UpdatePopulationFitness();
	// Copy the top ElitePop individuals to the new population
  ...
  RandomState TempRS;
    ...
    TempRS = RandomStates[k];
    RandomStates[k] = RandomStates[i];
    RandomStates[i] = TempRS;
	}
...
#ifdef TRACK_LINEAGES
inline int partition(int first, int last, TVector<double> &perf, TVector<TVector<double> > &pop, TVector<treeNode> &popNode)
{
	int pivot = first;
		...
		if (perf[i] > pivot_value) {
			...
			if (i != pivot) {
				...
				temp2 = pop[pivot]; pop[pivot] = pop[i]; pop[i] = temp2;
                                temp3 = popNode[pivot]; popNode[pivot] = popNode[i]; popNode[i] = temp3;
                       }
		}
	}
	temp1 = perf[pivot]; perf[pivot] = perf[first]; perf[first] = temp1;
	temp2 = pop[pivot]; pop[pivot] = pop[first]; pop[first] = temp2;
        temp3 = popNode[pivot]; popNode[pivot] = popNode[first]; popNode[first] = temp3;
        return pivot;
}
#else
...
#ifdef TRACK_LINEAGES
inline void quicksort(int first, int last, TVector<double> &perf, TVector<TVector<double> > &pop, TVector<treeNode> &popNode)
{
	if (first < last) {
		int pivot = partition(first,last,perf,pop,popNode);
		quicksort(first,pivot-1,perf,pop,popNode);
		quicksort(pivot+1,last,perf,pop,popNode);
	}
}
#else
...
{
    #ifdef TRACK_LINEAGES
        quicksort(1, Population.Size(), Perf, Population, PopulationNodes);
    #else
