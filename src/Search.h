...
// Uncomment the following line to track lineages (currently only works for hill-climbing reproduction)
//#define TRACK_LINEAGES
...
#ifdef TRACK_LINEAGES
struct treeNodeVal {
    TVector<double> individual;
    int generation;
};
typedef tree<treeNodeVal>::iterator treeNode;
#endif
...
const double SearchValueRange = MaxSearchValue - MinSearchValue;

// A utility function for folding a search parameter back into the allowable
// parameter range
inline double fold(double x, double min, double max)
{
    double range = max - min;
    if (x >= min && x <= max) return x;
    while (x <= min - range) x += range;
    while (x >= max + range) x -= range;
    if (x > max) x = 2*max - x;
    else if (x < min) x = 2*min - x;
    return x;
}
...
inline double LinearMapSearchParameter(double x, double min, double max, 
...
inline double ExpMapSearchParameter(double x, double minExp, double maxExp)
{
    double m = (maxExp - minExp)/(MaxSearchValue - MinSearchValue);
    double b = minExp - m * MinSearchValue;
...
class TSearch {
	public:
            ...
            void SetSearchFold(TVector<int> &Fold);
            void SetSearchFold(int Flag);
            ...
            #ifdef TRACK_LINEAGES
            treeNode Parent(treeNode &child) {return FamilyTree.parent(child);};
            treeNode BestNode(void) {return BestLeaf;};
            #endif
            // Control
	...
	private:
            ...
            #ifdef TRACK_LINEAGES
            tree<treeNodeVal> FamilyTree;
            TVector<treeNode> PopulationNodes;
            treeNode BestLeaf;
            #endif
            // Search Modes
